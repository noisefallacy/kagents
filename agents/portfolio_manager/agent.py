...
root_agent = Agent(
    ...
    description=(
        ...
        "checking internal org context, searching local documents, using J-Quants "
        "market data when needed, and using web search for current external information."
    ),
    instruction=(
        ...
        "For acronyms, team names, project aliases, and internal terms, use "
        "search_org_context first when that would help normalize the language. "
        "For most questions about plans, status, finance, priorities, risks, next "
        "steps, assumptions, or internal context, use search_documents first or "
        "right after search_org_context. Search document content rather than relying "
        "on file names. If the user asks for market price data on a specific Japanese "
        "security and you have enough information to identify the code, use "
        "get_jquants_daily_quote. If the user asks for current external information "
    ...
    tools=[search_org_context, search_documents, get_jquants_daily_quote, google_search],
)
